...
file.copy(fullpath, "murders1.csv", overwrite = T)
...
```{r}
file.copy(fullpath, "murders3.csv", overwrite=T)
...
cols(state = col_character(),abb = col_character(), region = col_character(), population = col_double(), total = col_double())
...
file.copy(fullpath, files[t], overwrite = T)
...
unlink(tmp_filename, recursive=F)
...
2^7 = 128
  unique items, enough to encode all the characters on an English language keyboard. However, other languages use characters not included in this encoding. For example, the e in Mexico is not encoded by ASCII. For this reason, a new encoding, using more than 7 bits, was defined: Unicode. When using Unicode, one can chose between 8, 16, and 32 bits abbreviated UTF-8, UTF-16, and UTF-32 respectively. RStudio actually defaults to UTF-8 encoding.
